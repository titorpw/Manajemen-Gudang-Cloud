...
flowchart TB
    ...
    CR ---|Unix Socket TLS| SQL[(🗄️ Cloud SQL<br/>MySQL 8.0<br/>db-f1-micro<br/>asia-southeast2)]

    CR ---|Store + Public URL| GCS[📦 Cloud Storage<br/>UBLA Bucket<br/>US Region<br/>Public Read]

    CR ---|Token Validation| FAuth[🔐 Firebase Auth<br/>Client-side SDK<br/>Stateless JWT]

    CR ---|at runtime| SM[🔑 Secret Manager<br/>APP_KEY<br/>DB_PASSWORD<br/>FIREBASE_CREDENTIALS]

    classDef edge fill:#B8860B,stroke:#333,stroke-width:2px,color:white
    classDef compute fill:#1E5A8E,stroke:#333,stroke-width:2px,color:white
    classDef storage fill:#5B2C8E,stroke:#333,stroke-width:2px,color:white
    classDef security fill:#2E7D32,stroke:#333,stroke-width:2px,color:white
    classDef user fill:#7B1FA2,stroke:#333,stroke-width:2px,color:white
...
flowchart TB
    subgraph Edge["🌐 Edge Layer"]
        ...
        CFDNS[Cloudflare DNS<br/>CNAME to Cloud Run]
    end
    ...
    subgraph Auth["🔐 Auth and Secrets"]
        FAuth[🔐 Firebase Auth<br/>max 50k MAU free]
...
sequenceDiagram
    ...
    CR->>CR: Cold start if instance=0<br/>entrypoint.sh then config:cache<br/>php artisan migrate --force

    alt Authenticated Request
        U->>CR: API call + Firebase JWT
        CR->>FA: Validate ID token<br/>local JWKs
        FA-->>CR: Token valid or invalid
    end

    CR->>SQL: Query via Unix socket
    SQL-->>CR: Result set

    opt Image Upload Staff
        U->>CR: POST /api/barang<br/>multipart/form-data
        CR->>GCS: store item_images gcs
        GCS-->>CR: Path returned
        CR->>SQL: INSERT image_url
    end

    opt Image Display
        CR->>GCS: url path - permanent public URL
        GCS-->>CR: storage.googleapis.com/BUCKET/path
        CR-->>U: JSON response with foto_url
    end

    CR-->>U: HTML/JSON response
```
